...
const getAqiColors = (category: string) => {
  ...
  if (normalized.includes('bardzo dobry')) return { bg: '#d4edda', text: '#155724' }; // Green
  if (normalized.includes('dobry')) return { bg: '#c3e6cb', text: '#155724' }; // Light Green
  if (normalized.includes('umiarkowany')) return { bg: '#fff3cd', text: '#856404' }; // Yellow
  if (normalized.includes('dostateczny')) return { bg: '#ffeeba', text: '#856404' }; // Orange
  if (normalized.includes('zły') && !normalized.includes('bardzo')) return { bg: '#f8d7da', text: '#721c24' }; // Red
  if (normalized.includes('bardzo zły')) return { bg: '#e2d9f3', text: '#4a148c' }; // Purple
...
export default function HomeScreen() {
  ...
  const renderIndicator = (name: string, indicatorData: { value: number | '-'; category: string }, textColor: string) => (
    <View style={styles.indicatorContainer}>
      ...
      <Text style={[styles.indicatorCategory, { color: textColor }]} numberOfLines={1} adjustsFontSizeToFit>{indicatorData.category}</Text>
    </View>
  );
      ...
      <View style={[styles.card, { backgroundColor: colors.bg }]}>
        <View style={styles.cardHeader}>
          <View style={styles.stationInfo}>
            ...
            <Text style={[styles.stationLocation, { color: colors.text }]}>{item.location}</Text>
            <Text style={[styles.stationDistance, { color: colors.text }]}>Distance: {item.distance.toFixed(1)} km</Text>
          </View>
          <View style={styles.aqiContainer}>
            <Text style={[styles.aqiValue, { color: colors.text }]}>{item.overallAqi}</Text>
            <Text style={[styles.aqiLabel, { color: colors.text }]}>AQI Index</Text>
          </View>
        </View>

        <View style={styles.statusContainer}>
          <Text style={[styles.statusText, { color: colors.text }]}>{item.overallQuality}</Text>
        </View>

        <View style={[styles.indicatorsGrid, { borderTopColor: `${colors.text}33` }]}>
  ...
  if (loading) {
    return (
      <ThemedView style={styles.centerContainer}>
        ...
        <ThemedText style={{ marginTop: 16 }}>Finding closest stations...</ThemedText>
      </ThemedView>
    );
  }
  ...
  if (error) {
    return (
      <ThemedView style={styles.centerContainer}>
        <ThemedText type="subtitle" style={{ color: 'red', textAlign: 'center', marginBottom: 16 }}>{error}</ThemedText>
        <ThemedText onPress={loadData} type="link">Try Again</ThemedText>
      </ThemedView>
    );
  }
